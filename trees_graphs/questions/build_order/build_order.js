 ...
 *
 * Solution: Visualising the problem as a directed graph is the best approach.
 * So we can start by building a graph where each node represets a project with it's
 * outgoing edges representing the projects that depend on it. That is, if A has an edge
 * to B (A -> B), it means B has a dependency on A and A must be built before B.
 *
 * The we use a topological soring helper to recursively process every node and update the
 * build order.
 *
 * We keep track of nodes we've visited so we don't repeat ourselves.
 * We also track of paths and check for cyclic dependencies
 *
 */
 ...
 * Build Order
 *
 * Time - O(P + D), where P is the number of projects and D is the number of dependencies.
 *
