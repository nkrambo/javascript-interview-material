...
describe.skip('singleSource()', () => {
  ...
  test('should return the shortest path costs from source to all nodes', () => {
    ...
    // expect(result[1].cost).toBe(5);
    //
    // expect(result[2].node).toBe('C');
    // expect(result[2].cost).toBe(2);
    //
    // expect(result[3].node).toBe('D');
    // expect(result[3].cost).toBe(6);
  });
});

describe.skip('uniformCostSearch()', () => {
  test('should return the shortest path from start to end node', () => {
